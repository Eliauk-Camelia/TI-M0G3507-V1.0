 ...
 *  @brief      启用DMP功能特性。
 *  输入参数 mask 使用的宏定义如下：
 *  \n DMP_FEATURE_TAP               // 点击检测功能
 *  \n DMP_FEATURE_ANDROID_ORIENT   // Android方向检测功能
 *  \n DMP_FEATURE_LP_QUAT          // 低功耗四元数输出功能
 *  \n DMP_FEATURE_6X_LP_QUAT       // 6轴低功耗四元数输出功能
 *  \n DMP_FEATURE_GYRO_CAL         // 陀螺仪校准功能
 *  \n DMP_FEATURE_SEND_RAW_ACCEL   // 发送原始加速度计数据
 *  \n DMP_FEATURE_SEND_RAW_GYRO    // 发送原始陀螺仪数据
 *  
 *  \n 注意：DMP_FEATURE_LP_QUAT 和 DMP_FEATURE_6X_LP_QUAT 不能同时启用（互斥）。
 *  \n 注意：DMP_FEATURE_SEND_RAW_GYRO 和 DMP_FEATURE_SEND_CAL_GYRO 也不能同时启用（互斥）。
 *
 *  @param[in]  mask    要启用的功能掩码（多个功能可按位或）。
 *  @return     返回0表示成功。
 */
...
int dmp_enable_feature(unsigned short mask)
{
    ...
    /* TODO: 这些配置可以整合进默认的 DMP 镜像固件中 */

    /* 设置陀螺仪的积分缩放因子（GYRO_SF 是缩放因子） */
    tmp[0] = (unsigned char)((GYRO_SF >> 24) & 0xFF);
    tmp[1] = (unsigned char)((GYRO_SF >> 16) & 0xFF);
    tmp[2] = (unsigned char)((GYRO_SF >> 8) & 0xFF);
    tmp[3] = (unsigned char)(GYRO_SF & 0xFF);
    mpu_write_mem(D_0_104, 4, tmp);  // 写入 DMP 内存地址 D_0_104，4 字节

    /* 配置传感器数据发送到 FIFO */
    tmp[0] = 0xA3; // 默认值
    if (mask & DMP_FEATURE_SEND_RAW_ACCEL) {  // 如果启用发送原始加速度计数据
        tmp[1] = 0xC0;
        tmp[2] = 0xC8;
        tmp[3] = 0xC2;
    } else {  // 不发送原始加速度计
    ...
    if (mask & DMP_FEATURE_SEND_ANY_GYRO) {  // 如果启用发送陀螺仪数据（原始或校准）
        tmp[4] = 0xC4;
        tmp[5] = 0xCC;
        tmp[6] = 0xC6;
    } else {  // 不发送陀螺仪数据
    ...
    tmp[8] = 0xA3;
    tmp[9] = 0xA3;
    mpu_write_mem(CFG_15, 10, tmp);  // 写入配置寄存器 CFG_15

    /* 配置手势数据发送到 FIFO */
    if (mask & (DMP_FEATURE_TAP | DMP_FEATURE_ANDROID_ORIENT))  // 如果启用点击或方向检测
        tmp[0] = DINA20;
    else
        tmp[0] = 0xD8;
    mpu_write_mem(CFG_27, 1, tmp);  // 写入配置寄存器 CFG_27
    ...
    if (mask & DMP_FEATURE_SEND_ANY_GYRO) {
        if (mask & DMP_FEATURE_SEND_CAL_GYRO) {  // 发送校准后的陀螺仪数据
            tmp[0] = 0xB2;
            tmp[1] = 0x8B;
            tmp[2] = 0xB6;
            tmp[3] = 0x9B;
    ...
    if (mask & DMP_FEATURE_TAP) {
        ...
        dmp_set_tap_thresh(TAP_XYZ, 250);     // 设置点击阈值
        dmp_set_tap_axes(TAP_XYZ);                   // 设置检测轴向（X,Y,Z）
        dmp_set_tap_count(1);                    // 设置点击计数
        dmp_set_tap_time(100);                // 设置点击时间
        dmp_set_tap_time_multi(500);          // 多次点击时间间隔

        dmp_set_shake_reject_thresh(GYRO_SF, 200);    // 设置抖动排除阈值
        dmp_set_shake_reject_time(40);                     // 设置抖动排除时间
        dmp_set_shake_reject_timeout(10);                  // 设置抖动排除超时时间
    } else {
        /* 关闭点击检测 */
        tmp[0] = 0xD8;
        mpu_write_mem(CFG_20, 1, tmp);
    ...
    mpu_reset_fifo();  // 重置 FIFO

    /* 计算每帧数据包的长度 */
    dmp.packet_length = 0;
    if (mask & DMP_FEATURE_SEND_RAW_ACCEL)
        dmp.packet_length += 6;  // 6字节加速度计数据
    if (mask & DMP_FEATURE_SEND_ANY_GYRO)
        dmp.packet_length += 6;  // 6字节陀螺仪数据
    if (mask & (DMP_FEATURE_LP_QUAT | DMP_FEATURE_6X_LP_QUAT))
        dmp.packet_length += 16; // 16字节四元数数据
    if (mask & (DMP_FEATURE_TAP | DMP_FEATURE_ANDROID_ORIENT))
        dmp.packet_length += 4;  // 4字节手势数据
