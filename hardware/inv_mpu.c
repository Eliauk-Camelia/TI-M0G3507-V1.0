/*
 $License:
    版权所有 (C) 2011-2012 InvenSense 公司，保留所有权利。
    详细授权信息请参见附带的 License.txt 文件。
 $
*/
/**
 *  @addtogroup  DRIVERS  传感器驱动层
 *  @brief       用于通过 I2C 通讯与传感器交互的硬件驱动。
 *
 *  @{
 *      @file       inv_mpu.c
 *      @brief      一款基于 I2C 协议的 Invensense 陀螺仪驱动。
 *      @details    该驱动支持以下设备：
 *                  MPU6050
 *                  MPU6500
 *                  MPU9150（或 MPU6050 + AK8975 磁力计辅助总线）
 *                  MPU9250（或 MPU6500 + AK8963 磁力计辅助总线）
 */
/* ★ 写在源码里, 不被 SysConfig 覆盖 ★ */
...
#define log_i(...)     do {} while (0)
#define log_e(...)     do {} while (0)
// #define log_i(fmt, ...) do { Sprintf(fmt, ##__VA_ARGS__); } while(0)
// #define log_e(fmt, ...) do { Sprintf(fmt, ##__VA_ARGS__); } while(0)
/* labs is already defined by TI's toolchain. */
/* fabs is for doubles. fabsf is for floats. */
#define fabs        fabsf
#define min(a,b) ((a<b)?a:b)
...
// 如果定义了 MPU9150（MPU6050 + 外接磁力计 AK8975）
#if defined MPU9150
    ...
    // 检查是否错误地定义了 AK8963（MPU9150 使用的是 AK8975）
    #if defined AK8963_SECONDARY
    #error "MPU9150 和 AK8963_SECONDARY 不能同时定义。"
    
    // 如果未定义 AK8975，则自动定义它（AK8975 是 MPU9150 的磁力计）
    #elif !defined AK8975_SECONDARY
    #define AK8975_SECONDARY
    #endif  /* #if defined AK8963_SECONDARY */

// 如果定义了 MPU9250（MPU6500 + 内置磁力计 AK8963）
    ...
    // 检查是否错误地定义了 AK8975（MPU9250 使用的是 AK8963）
    #if defined AK8975_SECONDARY
    #error "MPU9250 和 AK8975_SECONDARY 不能同时定义。"

    // 如果未定义 AK8963，则自动定义它（AK8963 是 MPU9250 的磁力计）
...
// 如果启用了 AK8975 或 AK8963 中的任意一个，统一定义为 AK89xx_SECONDARY
#if defined AK8975_SECONDARY || defined AK8963_SECONDARY
#define AK89xx_SECONDARY
#else
// 否则，不使用磁力计（注释掉的警告信息：使用无磁力计的方案 InvenSense 赚得少）
